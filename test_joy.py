...
from src.private_config import CLIENT_ID, CLIENT_SECRET, USER_AGENT

_GIF_SUFFIXES = (".gifv", ".gif")
_IMG_SUFFIXES = (".png", ".jpg", ".jpeg", ".PNG", ".JPG", ".JPEG")
...
class HistoryQueue(Generic[T]):
    """Queue class, similar to the stdlib one but with the following properties:

    * Elements can't be added to the same queue twice (history)
    * `get` returns a random element from the queue, rather than the first element.
    * `add` is O(N), since a shuffle of the queue's contents is performed.
    """
    def __init__(self):
        self._queue: list[T] = list()
        self._known: set [T] = set ()
        
        self.get = self._queue.pop
        """Get the next item from the queue."""
        self.__len__ = self._queue.__len__
        """The number of items in the queue."""

    def add(self, element: T) -> None:
        """Add an element to the queue if it's new."""
        if element not in self._known:
            self._queue.append(element)
            self._known.add(element)
            random.shuffle(self._queue)


async def _write_content(
        conn: asyncpraw.Reddit, 
        name: str,
        imgs: HistoryQueue[Submission],
        gifs: HistoryQueue[Submission]
) -> NoReturn:
    logging.info("spawning subreddit:    %s", name)
    subreddit = Subreddit(conn, name)
    
    logging.info("reading content from:  %s", subreddit.display_name)
    async for submission in subreddit.top(limit=None):
        if "imgur" in submission.url: # Fuck these guys
            continue
        if submission.url.endswith(_IMG_SUFFIXES):
            imgs.add(submission)
        elif submission.url.endswith(_GIF_SUFFIXES):
            gifs.add(submission)


async def _monitor(imgs: set[Submission], gifs: set[Submission]) -> NoReturn:
    """Monitor the status of the content queues."""
    while not await asyncio.sleep(_MONITOR_INTERVAL):
        logging.info("content count: gifs: %s, imgs: %s", len(gifs), len(imgs))


async def fetch_content_loop(
        imgs:  HistoryQueue[Submission], 
        gifs:  HistoryQueue[Submission],
        names: Iterable[str],
) -> NoReturn:
    logging.info("initializing the fetch content loop...")
    taskmap: dict[str, asyncio.Task] = {}
    conn = asyncpraw.Reddit(client_id=CLIENT_ID, client_secret=CLIENT_SECRET,
                            user_agent=USER_AGENT)

    # Spawn tasks
    asyncio.create_task(_monitor(imgs, gifs))
    for name in names:
        taskmap[name] = asyncio.create_task(_write_content(conn, name, imgs, gifs))

    # Manage tasks
    exceptions = []
    while not await asyncio.sleep(_MONITOR_INTERVAL):
        for name, task in taskmap.items():
            if not task.done():
                continue
            
            exceptions.append((task, task._exception))

            # Task either died or finished - restart it!
            taskmap[name] = asyncio.create_task(_write_content(conn, name, imgs, gifs))

        if exceptions:
            logging.error("The following exceptions occurred: %s", exceptions)
            exceptions = []
'''
    assert align_assignment_expressions(static_code) == static_code
